...
String hand_shape(double dis1, double angle1, double dis2, double angle2) // 거리와 각도에 따른 손 모양 판별 함수 정의
{
	if ((dis1 > 20 && dis1 < 70) && (dis2 > 60 && dis2 < 85) && (angle1 > 0.3 && angle1 < 0.6) && (angle1 > 0.5 && angle1 < 0.9))
		// hull[0]~hull[1] 사이 거리: 20~70, hull[0]~hull[1] 사이 각도: 0.3~0.6, hull[1]~hull[2] 사이 거리: 60~85, hull[1]~hull[2] 사이 긱도: 0.5~0.9
	{
		...
		// hull[0]~hull[1] 사이 거리: 120~175, hull[0]~hull[1] 사이 각도: 0.9~1.1, hull[1]~hull[2] 사이 거리: 50~140, hull[1]~hull[2] 사이 긱도: 1.9이상
	{
